require_relative '../lib/scarpe_tui'

Scarpe.app(true, title: "Test Finale Scarpe-TUI (Fase 3: Claude Style)") do
  
  nome_input = nil
  lang_input = nil
  termini_checkbox = nil
  bio_input = nil
  output_area = nil

  border stroke: "cyan" do
    
    stack do
      para "=== GENERATORE DI PROFILI GSoC 2026 ===", stroke: "cyan", modifier: "bold"
      para "La form scorre in un'area dedicata, i bottoni sono ancorati in basso.", stroke: "blue"
      para "--------------------------------------------------------------------"
    end

    scroll_area max_height: 12 do
      para "Come ti chiami?"
      nome_input = edit_line("", stroke: "dark_yellow") 

      para "Qual e' il tuo linguaggio preferito?"
      lang_input = edit_line("", stroke: "dark_yellow")

      para " "

      border stroke: "magenta" do
        flow do
          termini_checkbox = checkbox stroke: "magenta"
          para " Accetto i termini e condizioni"
        end
      end

      border stroke: "blue" do
        stack do
          para "Scrivi una breve biografia (Usa INVIO per andare a capo):"
          bio_input = edit_box("") 
        end
      end

      para " "
      
      output_area = para "In attesa di input...", stroke: "dark_gray"
      para " " 
      para " "
    end

    dock_bottom do
      border stroke: "light_gray" do
        flow do 
          button "Genera Profilo", stroke: "white", fill: "dark_green" do
          
            nome = nome_input.text.strip
            linguaggio = lang_input.text.strip
            biografia = bio_input.text.strip
            accettato = termini_checkbox.checked?
            
            if nome.empty? || linguaggio.empty?
              output_area.text = "-> ERRORE: Compila tutti i campi prima di generare!"
            elsif !accettato
              output_area.text = "-> ERRORE: Devi accettare i termini e condizioni per proseguire!"
            else

              Thread.new do
                messaggio_finale = "*** NUOVO PROFILO GENERATO ***\n" \
                                   "Nome Utente: #{nome}\n" \
                                   "Linguaggio : #{linguaggio}\n" \
                                   "Qualifica  : Architetto di Interfacce Native\n" \
                                   "Termini    : Accettati\n" \
                                   "Biografia  :\n" \
                                   "#{biografia.empty? ? 'Nessuna biografia inserita.' : biografia}\n" \
                                   "******************************"
                
                testo_corrente = ""
                output_area.text = "Inizializzazione generazione..."
                
                messaggio_finale.each_char do |char|
                  testo_corrente += char
                  output_area.text = testo_corrente
                end
              end
            end
          end

          button "Esci", stroke: "white", fill: "dark_red" do
            quit
          end
        end
      end
    end

  end
end